...
const PhotoScreen = () => {
  ...
  /*<Link
  to="/home"
  className="absolute top-4 left-4 z-10 flex items-center gap-2 text-gray-700 hover:text-black transition"
>
  <ArrowLeftOutlined className="text-2xl" />
  className="overflow-y-auto flex-1 flex flex-col gap-1 space-y-3 !pr-2"
  className="overflow-y-auto flex-1 flex flex-col gap-1 space-y-3 !pr-2
</Link>*/
            ...
            {comments.map((c, i) => (
              <div key={i} className="flex gap-3 !p-2 rounded-md">
                ...
                <div className="flex flex-col gap-0.5 w-full break-words overflow-hidden">
                  ...
                  <span className="text-[10px] text-gray-500">{formatDate(c.data_criacao)}</span>
                </div>
              </div>
            ))}
          </div>
